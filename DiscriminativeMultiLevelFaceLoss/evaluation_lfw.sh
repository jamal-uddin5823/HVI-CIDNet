...
RESULTS_BASE="./results/discriminative/lfw"
LOG_DIR="./logs/discriminative/lfw"
...
LOG_FILE="${LOG_DIR}/evaluation_lfw_${TIMESTAMP}.log"
...
if [ ! -d "$DATASET_DIR" ]; then
    ...
    echo "  Run: python prepare_lfw_dataset.py --download" | tee -a "$LOG_FILE"
    exit 1
fi
...
if [ ! -f "$PAIRS_FILE" ]; then
    ...
    python generate_lfw_pairs.py --test_dir="$DATASET_DIR" --output="$PAIRS_FILE" --num_pairs 2000 2>&1 | tee -a "$LOG_FILE"
fi
...
echo "✓ LFW evaluation completed" | tee -a "$LOG_FILE"
