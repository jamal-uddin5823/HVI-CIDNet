...
def transform_lapaface(size=256):
    """Transform for LaPa-Face dataset - similar to LFW but for face images"""
    return Compose([
        Resize((size + 32, size + 32)),  # Resize to slightly larger than crop size
        RandomCrop((size, size)),
        RandomHorizontalFlip(),
...
def get_lapaface_training_set(data_dir, size):
    return LaPaFaceDatasetFromFolder(data_dir, transform=transform_lapaface(size))

def get_lapaface_eval_set(data_dir):
    # For eval, resize to standard size without random crop
