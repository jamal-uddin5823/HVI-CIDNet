...
def load_datasets():
    ...
    if opt.lol_v1 or opt.lol_blur or opt.lolv2_real or opt.lolv2_syn or opt.SID or opt.SICE_mix or opt.SICE_grad or opt.fivek or opt.lfw or opt.lapaface or opt.ssface:
        if opt.lol_v1:
            ...
            train_set = get_ssface_training_set(opt.data_train_ssface, size=opt.cropSize)
            training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=opt.shuffle)
            test_set = get_ssface_eval_set(opt.data_val_ssface)
            testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=1, shuffle=False)
    else:
...
if __name__ == '__main__':  
    ...
    for epoch in range(start_epoch+1, opt.nEpochs + start_epoch + 1):
        ...
        if epoch % opt.snapshots == 0:
            ...
            if opt.ssface:
                output_folder = 'ss_face/'
                label_dir = opt.data_valgt_ssface
                norm_size = True
